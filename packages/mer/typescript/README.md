...
Requires Node.js 20 or newer. The release exports types alongside ESM JavaScript and has no runtime npm dependency. The development build uses TypeScript; package contents are restricted to the compiled API, public data, README and license.

```typescript
import { verifyMer, verifyMerChain, signMerReceipt } from "@axiom-relay/mer";

const result = verifyMer(receiptEnvelope, {
  trustedKeys: configuredIssuerKeys,
  verificationTime: new Date().toISOString(),
});

const chain = verifyMerChain(envelopesIncludingPredecessors, {
  trustedKeys: configuredIssuerKeys,
});

const signed = await signMerReceipt(receipt, {
  trustedKey: configuredIssuerKey,
  sign: (canonicalBytes) => issuerSigningService.signEd25519(canonicalBytes),
});
```

The signing callback must return 64 raw signature bytes. Invalid schema, economics, issuer binding or key lifecycle is rejected before invoking the signer; the returned signature is then verified. No settlement or execution is inferred from a successful signing call.

The offline CLI accepts a receipt file and a separate trusted-key array. Its exit status is 0 for a valid receipt, 1 for verification failure, and 2 for invalid input or an execution error. The public example files work at their fixed fixture time.

Development reference: `pnpm build`, `pnpm test`; an independent package installation is also tested before publication.
